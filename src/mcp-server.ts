...
export class PersistentTerminalMcpServer {
  ...
  private setupTools(): void {
    ...
    this.server.tool(
      'write_terminal',
      'Write input to a terminal session. Commands will be automatically executed (newline added if not present).',
      {
      ...
      async ({ terminalId, input }): Promise<CallToolResult> => {
        try {
    ...
    this.server.tool(
      ...
      async ({ terminalId, since, maxLines, mode, headLines, tailLines }): Promise<CallToolResult> => {
        try {
