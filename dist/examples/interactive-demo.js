...
class InteractiveDemo {
        ...
        try {
            switch (command) {
                ...
                case 'send':
                    ...
                    break;
                case 'read':
                    await this.readOutput(args[1] ? parseInt(args[1]) : undefined, args[2] ? parseInt(args[2]) : undefined);
                    break;
                case 'kill':
    ...
    async selectTerminal(terminalId) {
        ...
        // 支持短 ID 匹配
        const result = await this.terminalManager.listTerminals();
        const terminal = result.terminals.find(t => t.id === terminalId || t.id.startsWith(terminalId));
        if (!terminal) {
